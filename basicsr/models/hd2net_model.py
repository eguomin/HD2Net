...
class HD2NetModel(VideoBaseModel):

    def __init__(self, opt):
        super(HD2NetModel, self).__init__(opt)
        if self.is_train:
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output,self.midlq= self.net_g(self.lq)
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for i in range(num_folders):
            ...
            self.output,self.midlq = apply(self.net_g,self.lq,input_shape=input_shape,overlap_shape=overlap_shape)
            ...
            if i < num_folders:
                ...
                midlq_out = visuals['midlq']
                midlq_out_img = midlq_out  # uint8, bgr
                # if 'midlq' in visuals:
                ...
                if save_img:
                    save_img_path = osp.join(self.opt['path']['visualization'],dataset_name,f'result',
                                             f'{img_name}.tif')
                    os.makedirs(osp.join(self.opt['path']['visualization'],dataset_name,f'result'),exist_ok=True)
                    tifffile.imwrite(save_img_path,result_img)

                    save_img_path = osp.join(self.opt['path']['visualization'],dataset_name,f'midlq',
                                             f'{img_name}.tif')
                    os.makedirs(osp.join(self.opt['path']['visualization'],dataset_name,f'midlq'),exist_ok=True)
                    tifffile.imwrite(save_img_path,midlq_out_img)
                # calculate metrics
                if with_metrics:
                    # calculate metrics
...
def apply(model, data, overlap_shape,input_shape):
    ...
    for image in data:
        ...
        midlq_image = np.zeros((1, *image_shape), dtype=np.float32)
        ...
        for block_idx in tqdm(list(product(*[range(ns) for ns in num_steps]))):
            ...
            midlq *= block_weight
            midlq_np = midlq.squeeze(0).cpu().numpy()
            # Write results back to images
            # print(result_image[0][0, :, :],weight_image[0, :, :])
            midlq_image[:,slice_in[0],slice_in[1],slice_in[2]] += midlq_np[:,slice_out[0],slice_out[1],slice_out[2]]
            # weight_image[slice_in] += block_weight[slice_out].cpu().numpy()
        ...
        midlq_image /= weight_image[ np.newaxis,...]
        midlq_image[np.isnan(midlq_image)] = 0
        midlq_out.append(midlq_image)

    return result if input_is_list else result[0],midlq_out
