...
from basicsr.data.data_util import duf_downsample, generate_frame_indices, read_img_seq, read_img_3d
...
@DATASET_REGISTRY.register()
class VideoTest3DDataset(data.Dataset):
    """Video test dataset.
    ...
    def __init__(self, opt):
        super(VideoTest3DDataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        self.gt_root, self.lq_root = opt['dataroot_gt'], opt['dataroot_lq']
        self.data_info = {'lq_path': [], 'gt_path': [], 'folder': [], 'idx': [], 'border': []}
        ...
        if opt['name'].lower() in ['vid4', 'reds4', 'redsofficial']:
            for subfolder_lq, subfolder_gt in zip(subfolders_lq, subfolders_gt):
                ...
                if self.cache_data:
                    logger.info(f'Cache {subfolder_name} for VideoTestDataset...')
                    self.imgs_lq[subfolder_name] = read_img_3d(img_paths_lq)
                    self.imgs_gt[subfolder_name] = read_img_3d(img_paths_gt)
                else:
                    self.imgs_lq[subfolder_name] = img_paths_lq
                    self.imgs_gt[subfolder_name] = img_paths_gt
        else:
...
class VideoRecurrentTestDataset(VideoTestDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cache_data:
            ...
            # imgs_lq = [read_img_seq(v) for v in img_paths_lq]
            #
            # imgs_gt = [read_img_seq(v) for v in img_paths_gt]
            imgs_lq = read_img_seq(img_paths_lq)
...
@DATASET_REGISTRY.register()
class Video3DTestDataset(VideoTest3DDataset):
    """Video test dataset for recurrent architectures, which takes LR video
    frames as input and output corresponding HR video frames.
    ...
    def __init__(self, opt):
        super(Video3DTestDataset, self).__init__(opt)
        # Find unique folder strings
        self.folders = sorted(list(set(self.data_info['folder'])))
    ...
    def __getitem__(self, index):
        ...
        if self.cache_data:
            ...
            #
            # imgs_gt = [read_img_seq(v) for v in img_paths_gt]
            imgs_lq = read_img_3d(img_paths_lq)
            imgs_gt = read_img_3d(img_paths_gt)
            imgs_gt.squeeze_(0)
            imgs_lq.squeeze_(0)
        return {
            'lq': imgs_lq,
