...
class REDSRecurrentDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if np.count_nonzero(img_lqs_np > self.intensity_threshold) < (np.prod(img_lqs_np.shape) *self.area_threshold):
            return {'lq': img_lqs, 'gt': img_gts, 'key': key,'valid':False}
        else:
            return {'lq': img_lqs, 'gt': img_gts, 'key': key,'valid':True}

    def __len__(self):
        return len(self.keys)

@DATASET_REGISTRY.register()
class REDS3DDataset(data.Dataset):
    """REDS dataset for training recurrent networks.
    ...
    def __init__(self, opt):
        super(REDS3DDataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])
        self.num_frame = opt['num_frame']
        if 'intensity_threshold' in opt:
